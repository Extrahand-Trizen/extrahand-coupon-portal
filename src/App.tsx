...
import { NotificationCampaignsPanel } from './components/NotificationCampaignsPanel';
import { flowsFromServiceIds, labelForSlug } from './data/categorySlugs';

type View = 'landing' | 'login' | 'admin';
type AdminTab = 'coupons' | 'categories' | 'sku-content' | 'help-support' | 'category-content' | 'notification-templates' | 'notification-campaigns';
...
function AdminDashboard({
  ...
  return (
    <div className={`mgmt-container ${isSidebarCollapsed ? 'sidebar-collapsed' : ''}`}>
      <aside className="mgmt-sidebar">
        <div className="sidebar-header">
          {!isSidebarCollapsed ? (
            <div className="mgmt-brand">
              ...
              <div>
                <strong>ExtraHand</strong>
                <span>Admin Portal</span>
              </div>
            </div>
          ) : (
            <img src="/logo.png" alt="" width={32} height={32} style={{ margin: '0 auto', display: 'block' }} />
          )}
          <button
        ...
        <nav className="sidebar-nav" aria-label="Admin sections">
          {(
            [
              [
                'coupons',
                <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" style={{ verticalAlign: 'middle' }}><path d="M2 9a3 3 0 0 1 0 6v2a2 2 0 0 0 2 2h16a2 2 0 0 0 2-2v-2a3 3 0 0 1 0-6V7a2 2 0 0 0-2-2H4a2 2 0 0 0-2 2Z"/><path d="M12 9v6"/></svg>,
                'Coupons'
              ],
              [
                'categories',
                <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" style={{ verticalAlign: 'middle' }}><path d="M20 20a2 2 0 0 0 2-2V8a2 2 0 0 0-2-2h-7.9a2 2 0 0 1-1.69-.9L9.6 3.9A2 2 0 0 0 7.93 3H4a2 2 0 0 0-2 2v13a2 2 0 0 0 2 2Z"/></svg>,
                'Categories'
              ],
              [
                'category-content',
                <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" style={{ verticalAlign: 'middle' }}><path d="M14.7 6.3a1 1 0 0 0 0 1.4l1.6 1.6a1 1 0 0 0 1.4 0l3.77-3.77a6 6 0 0 1-7.94 7.94l-6.91 6.91a2.12 2.12 0 0 1-3-3l6.91-6.91a6 6 0 0 1 7.94-7.94l-3.76 3.76z"/></svg>,
                'Services'
              ],
              [
                'sku-content',
                <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" style={{ verticalAlign: 'middle' }}><path d="M21 16V8a2 2 0 0 0-1-1.73l-7-4a2 2 0 0 0-2 0l-7 4A2 2 0 0 0 3 8v8a2 2 0 0 0 1 1.73l7 4a2 2 0 0 0 2 0l7-4A2 2 0 0 0 21 16z"/><path d="M3.27 6.96 12 12.01l8.73-5.05"/><path d="M12 22.08V12"/></svg>,
                'Packages / SKUs'
              ],
              [
                'help-support',
                <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" style={{ verticalAlign: 'middle' }}><circle cx="12" cy="12" r="10"/><path d="M9.09 9a3 3 0 0 1 5.83 1c0 2-3 3-3 3"/><line x1="12" y1="17" x2="12.01" y2="17"/></svg>,
                'Help & Support'
              ],
              [
                'notification-templates',
                <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" style={{ verticalAlign: 'middle' }}><path d="M14.5 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V7.5L14.5 2z"/><polyline points="14 2 14 8 20 8"/></svg>,
                'Templates'
              ],
              [
                'notification-campaigns',
                <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" style={{ verticalAlign: 'middle' }}><path d="M18 8A6 6 0 0 0 6 8c0 7-3 9-3 9h18s-3-2-3-9"/><path d="M13.73 21a2 2 0 0 1-3.46 0"/></svg>,
                'Campaigns'
              ],
            ] as const
          ).map(([id, icon, label]) => (
            <button
              key={id}
              type="button"
              className={`sidebar-nav-item ${adminTab === id ? 'sidebar-nav-item-active' : ''}`}
        ...
        >
          {isSidebarCollapsed ? (
            <div className="sidebar-user-collapsed" style={{ textAlign: 'center', fontSize: '1.25rem', paddingBottom: '4px' }} title={`Signed in as ${admin.username}`}>
              👤
            </div>
          ) : (
            <div className="sidebar-user">
              <span className="username">Signed in as {admin.username}</span>
            </div>
          )}
          <button
            ...
            type="button"
            onClick={(e) => {
              e.stopPropagation(); // Prevent trigger from expanding sidebar
              clearSession();
              onLogout();
            }}
            title="Logout"
      ...
      <div 
        className="mgmt-content-wrapper"
      ...
      >
        <main className="mgmt-body">
          ...
          {adminTab === 'notification-templates' ? <NotificationTemplatesPanel /> : null}
          {adminTab === 'notification-campaigns' ? <NotificationCampaignsPanel /> : null}
          {adminTab === 'coupons' ? (
          <div className="mgmt-card-container">
